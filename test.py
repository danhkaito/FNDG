...
data_test = load_pickle('../dataset/data_preprocess_imbalance_test')
model=load_pickle('./model/KSOM/ksom_model.ckpt')
PNodes_arr_test=create_pnode(model, data_test)
...
for i in range(0, len(data_test)):
    ...
    SuitNode, ix, iy = model.FindBestMatchingNode(PNodes_arr_test[i])
    weight_val= model.CalculateDistance_PNode2CNode(PNodes_arr_test[i], SuitNode)
    ...
    for node_idx in model.m_Som[iy, ix].PNodes:
        # print(f"Node {iy}, {ix}\n")
        weight_pnode2pnode=model.CalculateCosine2PNode(model.m_Som[iy, ix].PNodes[node_idx], PNodes_arr_test[i])
    ...
    features_content_test= torch.cat((features_content_test, torch.from_numpy(np.array([PNodes_arr_test[i].getvector(0)])).type(torch.float32)))
    ...
    model_fakenew.eval()
    pred = model_fakenew(features_content_test, features_style_test, edge_index_test, edge_type_test)  
    pred_labels=torch.argmax(pred, dim=-1).detach().cpu()[idx_start].item()
    # print(data_test[i][-1])
    # print(pred_labels[idx_start].item())
    if data_test[i][-1]==1 and pred_labels==1:
      TP+=1
    elif data_test[i][-1]==1 and pred_labels==0:
      FN+=1
    elif data_test[i][-1]==0 and pred_labels==0:
      TN+=1
    else:
      FP+=1
